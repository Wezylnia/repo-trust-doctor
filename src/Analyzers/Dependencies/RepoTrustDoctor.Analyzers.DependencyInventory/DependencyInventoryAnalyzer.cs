...
public sealed class DependencyInventoryAnalyzer : IRepositoryAnalyzer
{
    ...
    [
        ...
        new("TRUST-DEP025", "Direct Go dependency uses a pseudo-version", AnalysisCategory.Dependencies, Severity.Low, Confidence.High, "A direct Go module dependency references a pseudo-version.", "Prefer tagged releases over direct pseudo-version dependencies and review pseudo-version origins."),
        ...
        new("TRUST-DEP029", "Cargo dependency uses a non-exact version without lockfile", AnalysisCategory.Dependencies, Severity.Medium, Confidence.High, "A Cargo dependency does not use an exact pinned version and no Cargo.lock was found.", "Commit Cargo.lock for reproducible Cargo builds, or use exact versions when strict direct dependency pinning is required."),
        ...
        new("TRUST-DEP031", "Composer application does not have a composer.lock file", AnalysisCategory.Dependencies, Severity.Medium, Confidence.High, "A Composer application manifest exists but no composer.lock was found.", "Run 'composer install' and commit composer.lock to the repository for reproducible application builds."),
        new("TRUST-DEP032", "Composer application has unlocked version constraints", AnalysisCategory.Dependencies, Severity.Medium, Confidence.High, "A Composer application has version constraints without a lockfile.", "Commit composer.lock for applications; reusable Composer libraries can intentionally publish version ranges without a lockfile."),
        ...
        new("TRUST-DEP043", "Swift executable package does not have a Package.resolved file", AnalysisCategory.Dependencies, Severity.Medium, Confidence.High, "A Swift executable package exists but no Package.resolved was found.", "Commit Package.resolved for reproducible executable builds."),
