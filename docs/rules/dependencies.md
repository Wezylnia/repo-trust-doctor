...
Libraries declared in `libs.versions.toml` are also evaluated. Direct versions, shared `version.ref` entries, nested refs, and rich versions are resolved into Maven package coordinates. Missing refs and intentionally unversioned aliases remain visible as Medium-confidence unpinned dependencies. Dynamic catalog versions use `TRUST-DEP050` instead of receiving a duplicate `TRUST-DEP018` penalty.
...
Detects Cargo dependencies that do not use an exact requirement (e.g. `"1"`, `"1.2"`, or `"1.2.3"` instead of `"=1.2.3"`) when no covering `Cargo.lock` is present. The collector still records non-exact requirements in the dependency inventory when a same-directory or workspace-root lockfile exists, but it does not emit this finding because the lockfile provides the reproducibility signal for normal Cargo projects. Member declarations using `workspace = true` inherit their effective requirement and source from the nearest `[workspace.dependencies]` catalog; unused workspace definitions are not treated as installed packages. If a lockfile contains multiple versions of one crate, Cargo-compatible requirement matching must identify exactly one candidate before the package is marked resolved. Ambiguous candidates remain in metadata rather than being silently discarded or guessed. Exact `=x.y.z` requirements are normalized for metadata and advisory lookup. The collector reads normal dependency sections, target-specific dependency sections, and dependency subtables such as `[dependencies.serde]` without treating metadata keys like `features` as packages.
