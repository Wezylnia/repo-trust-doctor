...
- Maven and Gradle: `pom.xml`, `build.gradle`, `build.gradle.kts`, Java lock evidence, coordinate-specific Maven dependency-management resolution, conservative parent/BOM/platform signals, dynamic versions, snapshots/prereleases, Gradle wrapper evidence. Unrelated dependency management no longer hides missing versions; uncertain management lowers finding confidence instead of suppressing it.
